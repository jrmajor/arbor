person = osoba

index =
    .by_family_name = Po nazwisku rodowym
    .by_last_name = Po nazwisku
    .total = łącznie

menu =
    .overview = Widok osoby

    .edit_person = Edytuj osobę
    .edit_biography = Edytuj biografię
    .edits_history = Historia edycji

    .make_invisible = Sprywatyzuj
    .make_visible = Opublicznij

    .delete = Usuń
    .delete_confirm = Czy na pewno chcesz usunąć tę osobę?
    .restore = Przywróć

titles =
    .index = Indeks
    .new_person = Nowa osoba
    .editing = Edycja
    .biography_editing = Edycja biografii
    .edits_history = Historia edycji

alerts =
    .person_has_been_created = Osoba została utworzona.
    .changes_have_been_saved = Zmiany zostały zapisane.
    .visibility_has_been_changed = Widoczność została zmieniona.
    .cant_delete_person_with_marriages = Aby usunąć tę osobę, usuń najpierw jej małżeństwa.
    .cant_delete_person_with_children = Aby usunąć tę osobę, usuń najpierw jej dzieci.
    .person_has_been_deleted = Osoba została usunięta.
    .person_has_been_restored = Osoba została przywrócona.

history =
    .deleted = Osoba została usunięta.
    .restored = Osoba została przywrócona.
    .made_visible = Osoba została upubliczniona.
    .made_invisible = Osoba została ukryta.
    .added-biography = Biografia została dodana.
    .deleted-biography = Biografia została usunięta.
    .updated-biography = Biografia została zmodyfikowana.

add_a_new_person = Dodaj nową osobę

sex = Płeć
female = kobieta
male = mężczyzna
unknown = nieznana
external_links = Linki zewnętrzne
id_in = ID w
pytlewski =
    .as = jako
    .show_more = pokaż więcej
    .parents = Rodzice
    .marriages = Małżeństwa
    .children = Dzieci
    .siblings = Rodzeństwo
    .not_found = nie znaleziono
wielcy =
    .as = jako
name = Imię
middle_name = Kolejne imiona
names = Imiona
family_name = Nazwisko rodowe
last_name = Nazwisko zmienione
birth = Urodzenie
birth_date = Data urodzenia
birth_place = Miejsce urodzenia
current_age = aktualny wiek
estimated_birth_date = szacowana data urodzenia
death = Zgon
dead = Zmarły
    .xx = Zmarła
    .xy = Zmarły
death_date = Data zgonu
death_place = Miejsce zgonu
death_cause = Powód zgonu
death_age = wiek w chwili zgonu
funeral = Pogrzeb
funeral_date = Data pogrzebu
funeral_place = Miejsce pogrzebu
burial = Pochówek
burial_date = Data pochówku
burial_place = Miejsce pochówku
parents = Rodzice
mother = Matka
father = Ojciec
siblings = Rodzeństwo
siblings_mother = Rodzeństwo od str. matki
siblings_father = Rodzeństwo od str. ojca
marriages = Małżeństwa
children = Dzieci
sources = Źródła
isbn_url = https://pl.wikipedia.org/wiki/Specjalna:Książki/
isbn_in_wikipedia = w Wikipedii
